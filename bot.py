...
async def load_futures_symbols():
    """Завантажує валідні PERPETUAL символи у кеш."""
    try:
        s = await get_http()
        async with s.get(BINANCE_EXCHANGE_INFO) as r:
            r.raise_for_status()
            data = await r.json()
        syms = set()
...
async def fetch_funding(symbol: str):
    """Повертає (symbol, funding_decimal, next_kyiv_dt, mark_price, last_price, change_pct)."""
    s = await get_http()

    # premiumIndex
    async with s.get(BINANCE_PREMIUM, params={"symbol": symbol}) as r:
        r.raise_for_status()
        prem = await r.json()
    ...
    # 24h ticker
    async with s.get(BINANCE_24H, params={"symbol": symbol}) as r2:
        r2.raise_for_status()
        t24 = await r2.json()
    last = float(t24.get("lastPrice") or 0.0)
    chg = float(t24.get("priceChangePercent") or 0.0)

    return symbol, rate, next_dt, mark, last, chg
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = (
        "👋 Привіт!\n\n"
        "Команди:\n"
        "• /rate SYMBOL — показати funding (напр. /rate BTCUSDT, /rate 0GUSDT)\n"
        "• /alarm SYMBOL INTERVAL — надсилати регулярно (напр. /alarm BTCUSDT 30m)\n"
...
async def alarm_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if len(context.args) < 2:
        ...
        return
    sym = context.args[0].upper()
    interval_s = parse_interval(context.args[1])
    if not interval_s:
...
async def main():
    ...
    # завантажимо список символів та оновлюватимемо раз на 6 год
    await load_futures_symbols()
    app.job_queue.run_repeating(lambda c: asyncio.create_task(load_futures_symbols()),
                                interval=6*60*60, first=6*60*60,
    ...
    # запуск polling (Starter/Worker ніколи не засинає)
    await app.initialize()
    await app.start()
    logging.info("Bot is polling…")
    await app.updater.start_polling()
    await asyncio.Future()  # не завершуємося
